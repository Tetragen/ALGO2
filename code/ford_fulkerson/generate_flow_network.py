...
def generate_flow_network(nb_nodes, nb_edges, capacity_max):
    ...
    for node in range(nb_nodes):
        if source_capacities[node]:
            ...
            print(
                f"link source to {node} with capacity {source_capacities[node]}.")
            # draw the edge
    ...
    for node in range(nb_nodes):
        if sink_capacities[node]:
            ...
            # if sum(inner_capacities[:, node]):
            print(
                f"link {node} to sink with capacity {sink_capacities[node]}.")
            # draw the edge
    ...
    max_scope = 70
    for node in range(nb_nodes):
        pos[f"i-{node}"] = (np.random.uniform(min_scope, max_scope),node/nb_nodes*100)
    print(pos)
