...
with open('data/exercise_2_successors', 'rb') as f:
    successors = pickle.load(f)

with open('data/exercise_2_edges', 'rb') as f:
...
dot = Graph(comment='Graph 2 : dominating set')
for edge in edges_list:
    dot.edge(str(edge[0]),
             str(edge[1]),
             color='darkolivegreen4',
             penwidth='1.1')
graph_name = 'images/greedy_2_dominate/greedy_0'
...
def highlight_node(node, dot, index):
    dot.node(str(node),
             ...
             penwidth='10')
    # visualize the graph
    graph_name = 'images/greedy_2_dominate/greedy_' + str(index)
    dot.render(graph_name)
